...
const DocumentRequestForm = () => {
  ...
  const handleDocumentChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedType = event.target.value as DocumentTypes; // Type assertion here
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const cidsArray = Object.values(requiredCIDs).filter(cid => cid !== null);
    const formDataObj = {
      doctype: selectedDocument,
      issuingAuthority: documentTypes[selectedDocument]?.issuingAuthority || "",
      message: formData.additionalInfo,
      cid:cidsArray.join(', '), // Fill this with appropriate data if needed
      receiver: walletId,
      status: "Pending",
    };
